...
- The outbox relay retries failing rows with an exponential backoff (1 minute up to 1 hour) and gives up after `outbox.max_attempts` attempts; the table stores `attempts`, `available_at`, `failed_at` and `last_error`, and `setup` adds them to existing tables. Attempts are counted before a message is sent (a row that crashes the process is given up eventually), and a run that could not send anything never gives a row up.
- An outbox check in `somework:cqrs:health` (given-up rows, due rows waiting more than 10 minutes); the relay logs to the `logger` service; `purge` deletes in batches of 1000.
- The outbox relay runs as a single instance when symfony/lock is installed (lock scoped to the configured `framework.cache.prefix_seed` or the project directory, the connection and the table, extended after every row) and stops after 5 consecutive send failures.
...
- `ValidateHandlerCountPass` checks commands and queries per bus and counts distinct services; a handler registered without a bus (e.g. a plain `#[AsMessageHandler]`) counts on every bus, and handlers of parent classes and interfaces count for the messages that inherit them.
...
- Environment variables are rejected, with a clear message, in every option the container compilation needs (dispatch modes, transports, buses, service ids); they remain allowed in `retry_strategy.jitter`/`max_delay`, `idempotency.ttl`, `outbox.table_name`/`max_attempts` and the `async.dispatch_after_current_bus` flags.
