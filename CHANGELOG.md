...
- Outbox: `OutboxWriter` (`@api`) stores a message in one call, encoded with the outbox serializer, once per transport an asynchronous dispatch of the message would use (`transports.command_async`/`event_async`, `#[Asynchronous(transport: ...)]`); `OutboxMessage::fromEnvelope()` (time-ordered UUIDv7 ids), `OutboxMessage::$attempts` and `$lastError`, `OutboxStorage::purgePublished()` and `OutboxStorage::recordAttempt()` and an `$excludedTransports` argument for `OutboxStorage::fetchUnpublished()` (breaking for custom `OutboxStorage` implementations, see UPGRADE.md),
...
- The outbox relay retries failing rows with an exponential backoff (1 minute up to 1 hour) and gives up after `outbox.max_attempts` attempts, or three times as many when the transport fails (`TransportException`); the table stores `attempts`, `available_at`, `failed_at` and `last_error`, and `setup` adds them to existing tables. Each attempt is claimed before the message is sent: a row that crashes the process is given up eventually, and overlapping relays skip each other's rows. Each run relays the new rows first, in the order they were stored, then the rows due for a retry, so rows that keep failing never hold up new ones; an attempt the process did not survive gets three times `outbox.max_attempts` and keeps the previous error. The transports take turns, so the backlog of one does not hold up the others. An index on `(published_at, failed_at, transport_name, available_at, created_at, id)` serves the relay and replaces the 0.4 index on `(published_at, created_at)`; `setup` builds it with `CREATE INDEX CONCURRENTLY` on PostgreSQL, drops the old index only afterwards, rebuilds an index a killed build left invalid, serialises concurrent setups with a database lock (a table that is up to date takes no lock), and gives up after 5 seconds instead of blocking writes behind an open transaction. The automatic setup only creates the table or, for the relay and the `failed` command, adds the columns (storing never changes an existing table; it waits at most 1 second for open transactions, in one transaction on PostgreSQL, so it is safe behind PgBouncer) and leaves the index to `setup`; `DbalOutboxStorage::pendingChanges()` lists what `setup` still has to do, and the relay and the health check warn until it is done (meanwhile the relay fetches with one query along the index of 0.4). Adding the columns automatically does not wait in the lock queue of the table (`LOCK TABLE … NOWAIT` on PostgreSQL, where only an autovacuum that gives way is waited for, for `deadlock_timeout`; `ALTER TABLE … NOWAIT` on MariaDB; a check of the server's transactions on MySQL) and only adds them when that takes no time (`ALGORITHM=INSTANT` on MySQL and MariaDB); the health check reports a table of 0.4 as a warning (critical once messages have waited there for more than 10 minutes). The setup command says when it waits for another setup, often notices a transaction pooler on PostgreSQL (and refuses, or reports the lock it left behind), stops at once while another process builds the index, and exits with `128 + signal` when it is interrupted. `database.table` names work on MySQL and MariaDB, and an unqualified name is found along the PostgreSQL search path.
- The outbox relay pauses a transport that fails 3 times in a row (10 times, or 3 over 10 seconds, once it accepted a message in the run) until the next run and keeps relaying the other transports; SIGTERM and SIGINT stop it after the current row with exit code 1; after a PHP fatal error it still releases its lock.
- An outbox check in `somework:cqrs:health` (given-up rows, rows that keep failing, due rows waiting more than 10 minutes, a table that needs `somework:cqrs:outbox:setup`); the relay logs to the `logger` service; `purge` deletes in batches of 1000.
...
- **Breaking:** outbox storage contract v2: the relay claims fetched rows in batches with a token (`claim()`, `release()`, `recordFailure()`, `markPublished(array $ids)` instead of `recordAttempt()` and `markPublished(string $id)`); the table gains `claim_token`, `claimed_at` and `signature`, which writes need (run `outbox:setup` before deploying); an interrupted attempt is recognised by its claim and retried alone, and keeps the real error of the attempt before; sent rows are marked as published every 2 seconds.
- Outbox rows are signed (HMAC-SHA256, `outbox.signing`, on by default with `framework.secret`): the relay only decodes rows with a valid signature, so rows written around the application (e.g. through an SQL injection) never reach `unserialize()`; `previous_secrets` for rotation, `accept_unsigned` while rows of 0.4 drain, and `outbox:failed --requeue --sign <ids>` for rows an operator checked. `outbox:failed` shows the message class from the `type` header.
- Faster relay fetch with many transports: the transport list is kept for the run, each transport is read only as deep as the batch needs, and up to 50 transports are read in one `UNION ALL` statement (1.14 instead of 5.6 statements per row with 100 transports).
- Outbox status and health: counts stop at 10 000 rows (`OutboxStatus::$capped`), the oldest due row is found along the index per transport, unfinished claims are reported (`$inFlight`, `$oldestClaim`), and the health check warns about a claim older than 10 minutes; `retrying` counts the rows whose last attempt failed.
...
- `somework:cqrs:generate` wrote files outside the PSR-4 layout (a namespace no prefix covers is now refused unless `--dir` is given), accepted `..` and invalid class names, could escape the project directory through a sibling path prefix or a symlinked file, generated code that did not compile when a class name clashed with an import, and left half of a skeleton behind on failure.
