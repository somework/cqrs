...
- `outbox:failed --requeue --sign <ids>` shows the class in each body next to its type header, every class the body would instantiate and a digest of the body; it refuses to sign a row whose type header names another class, whose message is not a command, query or event, whose body instantiates a class the message and its stamps do not declare (`--allow-class` adds one), or that uses custom serialization (`Serializable`), and signs only the bodies it showed. The body is read by a parser of PHP's serialize() format, never unserialized. `--transport` needs the ids of the messages.
