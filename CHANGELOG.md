...
- The outbox relay retries failing rows with an exponential backoff (1 minute up to 1 hour) and gives up after `outbox.max_attempts` attempts, or three times as many when the transport fails (`TransportException`); the table stores `attempts`, `available_at`, `failed_at` and `last_error`, and `setup` adds them to existing tables. Each attempt is claimed before the message is sent: a row that crashes the process is given up eventually, and overlapping relays skip each other's rows. Rows are relayed in the order they became due, so a retried row queues up behind the rows stored before its retry time.
