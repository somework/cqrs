...
- A message with a `DeduplicateStamp` (from an `IdempotencyStamp`, its default stamps or the caller) is refused when it is stored in the outbox and the lock store ties its keys to the process (`flock`, `semaphore`, advisory locks), since the relay could never send it.
