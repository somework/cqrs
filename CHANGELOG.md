...
- Outbox: `OutboxMessage::fromEnvelope()` (time-ordered UUIDv7 ids), `OutboxMessage::$attempts` and `$lastError`, `OutboxStorage::purgePublished()` and `OutboxStorage::recordAttempt()` and an `$excludedTransports` argument for `OutboxStorage::fetchUnpublished()` (breaking for custom `OutboxStorage` implementations, see UPGRADE.md),
