...
- The relay claims each fetched batch with a token of its run before sending. A row whose attempt was interrupted (the process died) is retried on its own, keeps the error of the attempt before, and is given up after three times `max_attempts`. Unattempted claims are released, and sent rows are marked as published every 2 seconds.
...
- Capability interfaces `Contract\Outbox\OutboxSchema`, `FailedOutboxMessages` and `OutboxMonitoring`, with the `FailedOutboxMessage` and `OutboxStatus` DTOs. With them, setup, failed and health work with any storage, also behind a decorator.
...
- An outbox check in `somework:cqrs:health`: given-up rows, failing rows, due rows waiting more than 10 minutes, claims older than 10 minutes, and a table that needs the setup command. Counts stop at 10 000 rows.
- The index `idx_<table>_pending` for the relay. `setup` builds it with `CREATE INDEX CONCURRENTLY` on PostgreSQL, and serialises concurrent setups with a database lock. It gives up after 5 seconds instead of blocking writes, notices a transaction pooler, and exits with `128 + signal`.
...
- The outbox table gains seven columns and a new index, which writes need: run `somework:cqrs:outbox:setup` before deploying.
...
- The relay reuses the transport list for the run and reads up to 50 transports in one `UNION ALL` statement.
...
- `OutboxStorage::recordAttempt()` and the relay's "did not finish" marker in `last_error`.
...
- The outbox:
  ...
  - on Symfony 8, marked undecodable rows as published;
