...
- The relay resets the application's services (`services_resetter`) after each row it handled in its own process (no transport, `sync://`), like a Messenger worker; `--no-reset` turns that off.
- `outbox.relay_on_terminate` (for `when@dev`): the relay runs right after a request, a console command or a worker message (after its acknowledgement) that stored messages in the outbox, so development does not need a scheduled relay (with `sync://`, messages are handled at once). It waits while a transaction is open on the outbox connection, and skips interrupted commands.
