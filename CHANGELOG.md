...
- The outbox relay retries failing rows with an exponential backoff (1 minute up to 1 hour) and gives up after `outbox.max_attempts` attempts, or three times as many when the transport fails (`TransportException`); the table stores `attempts`, `available_at`, `failed_at` and `last_error`, and `setup` adds them to existing tables. Each attempt is claimed before the message is sent: a row that crashes the process is given up eventually, and overlapping relays skip each other's rows. Each run relays the new rows first, in the order they were stored, then the rows due for a retry, so rows that keep failing never hold up new ones; an attempt the process did not survive gets three times `outbox.max_attempts` and keeps the previous error. An index on `(published_at, failed_at, available_at, created_at, id)` serves the relay and replaces the 0.4 index on `(published_at, created_at)`.
- The outbox relay pauses a transport that fails 3 times in a row until the next run and keeps relaying the other transports; SIGTERM and SIGINT stop it after the current row with exit code 1; after a PHP fatal error it still releases its lock.
