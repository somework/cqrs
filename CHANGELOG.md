...
- `somework:cqrs:outbox:relay --watch` keeps relaying until `SIGTERM`/`SIGINT` or `--time-limit` (waiting `--sleep` seconds when nothing is due), for a process manager, and resets the services between runs like a Messenger worker.
- `outbox.relay_on_terminate` (for `when@dev`): the relay runs right after a request, a console command or a worker message that stored messages in the outbox, so development does not need a scheduled relay (with `sync://`, messages are handled at once).
