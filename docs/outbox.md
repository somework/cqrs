...
- **The transaction is checked** before the stamp pipeline and the middleware run. With
  ...
  `auto_commit: false` is always in a transaction. On it, the relay and the maintenance commands
  commit their own writes, the setup runs, and each message the relay handles in its own process
  (see *The transports*) is a unit of work of its own: a failed handler's writes are rolled back.
